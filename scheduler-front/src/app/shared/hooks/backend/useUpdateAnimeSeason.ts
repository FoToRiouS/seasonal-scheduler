...
const update = async (animeSeason: IAnimeSeason) : Promise<IAnimeSeason> => {
    const res = await SchedulerBackApi().put(`/animeseason/${animeSeason.id ? animeSeason.id : ""}`, animeSeason);
    return res.data
...
export function useUpdateAnimeSeason(idAnime: number) {
    ...
    return useMutation({
        ...
        onSuccess: (data) => {
            ...
            if(data.seasons){
                data.seasons.forEach(s => queryClient.invalidateQueries(["anime-season-by-season", s.year, s.season]))
            }
        }
    });
}
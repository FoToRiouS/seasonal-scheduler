...
const saveAnimeSeason = async (idAnime: number, year: number, season: AnimeSeasons) : Promise<IAnimeSeason> => {
    const { data: response } = await SchedulerBackApi().post("/animeseason/", {idAnime: idAnime, season: season, year: year});
    return response;
}
